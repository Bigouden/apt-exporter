...
APT_EXPORTER_NAME = os.environ.get('APT_EXPORTER_NAME',
                                       'apt-exporter')
APT_EXPORTER_LOGLEVEL = os.environ.get('APT_EXPORTER_LOGLEVEL',
                                           'INFO').upper()
...
try:
    ...
    logging.Formatter.converter = lambda *args: \
                                  datetime.now(tz=pytz.timezone(APT_EXPORTER_TZ)).timetuple()
...
METRICS = [
    {'name': 'apt_package',
     'description': 'APT Package Information',
     'type': 'gauge'}
]
